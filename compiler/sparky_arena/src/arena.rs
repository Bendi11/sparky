...
impl<T, Id: ArenaKey> Arena<T, Id> {
    ...
    pub fn get(&self, key: Id) -> &T {
        let key = key.index();
        if key >= self.vec.len() {
            panic!("Attempt to access arena element using an invalid key with index: {}", key);
        }
    ...
    pub fn get_mut(&mut self, key: Id) -> &mut T {
        let key = key.index();
        if key >= self.vec.len() {
            panic!("Attempt to mutable access arena element using an invalid key with index: {}", key);
        }
...
impl<T, I: ArenaKey> ops::Index<I> for Arena<T, I> {
    ...
    #[inline]
    fn index(&self, index: I) -> &Self::Output { self.get(index) }
}

impl<T, I: ArenaKey> ops::IndexMut<I> for Arena<T, I> {
    #[inline]
    fn index_mut(&mut self, index: I) -> &mut Self::Output { self.get_mut(index) }
}
