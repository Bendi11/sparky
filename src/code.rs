...
use inkwell::{builder::Builder, context::Context, module::{Linkage, Module}, types::BasicType, values::{AnyValue, AnyValueEnum, BasicValue, BasicValueEnum, FunctionValue}};
...
impl<'ctx> Compiler<'ctx> {
    ...
    pub fn gen_fn_proto(&mut self, proto: FnProto<'ctx>) -> Result<FunctionValue<'ctx>, Box<dyn std::error::Error>> {
        Ok( self.module.add_function(proto.name.as_str(), proto.ret.fn_type(proto.args.as_slice(), false), None) )
    } 
    ...
    fn gen(&mut self, ast: Ast<'ctx>) -> Result<AnyValueEnum<'ctx>, Box<dyn std::error::Error>> {
        ...
        match ast {
            ...
            Ast::StrLiteral(literal) => Ok( unsafe{ self.builder.build_global_string(literal.as_str(), "global_str_literal").as_any_value_enum() } ),
    ...
    pub fn gen_fn_decl(&mut self, proto: FnProto<'ctx>, body: Body<'ctx>) -> Result<FunctionValue<'ctx>, Box<dyn std::error::Error>> {
...
mod tests {
    ...
    pub fn codegen() {
        ...
        let mut module = gen.gen_all(parser.parse().unwrap()).unwrap();
        module.print_to_file("./o.il").unwrap();
