...
use crate::{util::files::FileId, ir::{FunId, IrStmt, value::IrIntegerValue, IrBB, BBId, IrFun, VarId, IrVar}, ast::{Stmt, StmtNode, Expr, ExprNode, ParsedModule}, Symbol};

use super::{IrLowerer, IntermediateModuleId, ScopePlate};
...
impl<'files, 'ctx> IrLowerer<'files, 'ctx> {
                ...
                if let ExprNode::Access(ref var) = let_stmt.let_expr.node {
                    ...
                    if var.len() == 1 && !self.lookup_var(&var.last()).is_none() {
                        ...
                        let ty = match &let_stmt.ty {
                            Some(ty) => self.resolve_type(ty, module, parsed, self.current_fun().file, stmt.span)?,
                            None => match &let_stmt.assigned {
                                Some(assigned) => unimplemented!(),
                                None => return Err(Diagnostic::error()
                                    .with_message(format!("Failed to infer the type of variable {}", name))
                                    .with_labels(vec![
                                        Label::primary(self.current_fun().file, stmt.span)
                        ...
                        let var = self.current_fun_mut().body.as_mut().unwrap().vars.insert(IrVar {
                            name,
                            ty, 
                        });
    ...
    fn current_scope_mut(&mut self) -> &mut ScopePlate {
        self.scope_stack.last_mut().expect("Internal compiler error: scope stack is empty")
    }
    
    /// Get an immutable reference to the current scope plate
    fn current_scope(&self) -> &ScopePlate {
        self.scope_stack.last().expect("Internal compiler error: scope stack is empty")
    }
    
    /// Get an immutable reference to the currently lowered function
    fn current_fun(&self) -> &IrFun {
        &self.ctx.funs[self.current_fun.expect("Internal compiler error: current function is none")]
    }
    
    /// Get a mutable reference to the currently lowered function
    fn current_fun_mut(&mut self) -> &mut IrFun {
        &mut self.ctx.funs[self.current_fun.expect("Internal compiler error: current function is none")]
    }
}
