...
use inkwell::{AddressSpace, IntPredicate, builder::Builder, context::Context, module::Module, types::{AnyTypeEnum, BasicType, BasicTypeEnum, StructType}, values::{AnyValue, AnyValueEnum, BasicValueEnum, FunctionValue, PointerValue}};
    ...
    pub fn gen(&mut self, node: &AstPos, lval: bool) -> Option<AnyValueEnum<'c>> {
        match node.ast() {
            ...
            Ast::Cast(expr, ty) => {
                ...
                Some(match (lhs.get_type(), self.llvm_type(ty, &node.1)) {
                    ...
                    (AnyTypeEnum::PointerType(_), BasicTypeEnum::PointerType(ptr2)) => self
                        ...
                        .as_any_value_enum(),
                    (AnyTypeEnum::ArrayType(_), BasicTypeEnum::PointerType(_)) => unsafe { self   
                        .build 
                        .build_in_bounds_gep(lhs.into_pointer_value(), &[self.ctx.i64_type().const_zero()], "array_to_ptr_cast_gep")
                        .as_any_value_enum()
                    }
            ...
            Ast::Array(expr, idx) => {
                ...
                if let Type::Integer{signed: _, width: _} = idx.get_type(self)? {

                } else {
                    error!("{}: Cannot index array with expression of type {}", node.1, idx.get_type(self)?);
                    return None
                }
 
                let ptr = if let Type::Array(ty, _) = ty {
                    let ptr = self.gen(expr, true)?.into_pointer_value();
                    self.build.build_bitcast(ptr, self.llvm_type(ty.deref(), &node.1).ptr_type(AddressSpace::Generic), "array_index_bitcast_to_ptr")
                        .into_pointer_value()
                ...
                let res = Some(match lval {
                    true => ptr.as_any_value_enum(),
                    false => self.build.build_load(ptr, "array_index_load_idx").as_any_value_enum()
                });
